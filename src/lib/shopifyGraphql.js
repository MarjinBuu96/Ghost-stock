...
export async function getInventoryByVariantGQL(
  shop,
  accessToken,
  { multiLocation = false, pageSize = 250, levelPageSize = 250 } = {}
) {
  const query = `
    query Variants($first: Int!, $after: String${multiLocation ? ', $levelFirst: Int!' : ''}) {
  ...
  while (hasNext) {
    const variables = {
      first: pageSize,
      after,
      ...(multiLocation ? { levelFirst: levelPageSize } : {}),
    };

    const data = await gql(shop, accessToken, query, variables);
